...
def plot_input(ax, pars, I, title):
    """
    Plots input current.
    ...
    plt.title(title)
    plt.show()'''
    ax.set_ylabel('I', color='blue')

    T = pars['T']
    range_t = pars['range_t']
    ax.plot(range_t, I, label='I', color='green')
    ax.tick_params(axis='y')


def plot_voltage(ax, pars, v, title):
    """
...
plot_input(axs[0], p, I_delta, "Delta input current")
plot_voltage(axs[1], p, v2, ", delta input current")
plot_spikes(axs[2], spikes_ms2, p['range_t'])
